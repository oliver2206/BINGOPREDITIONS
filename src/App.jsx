...
export default function App() {
  ...
  const [previousRounds, setPreviousRounds] = useState(new Set())
  // how many rounds each number has been called in, e.g. { 6: 2 } -> shown as "2x"
  ...
  const callNumber = (n) => {
    ...
    setCalledThisRound((prev) => new Set(prev).add(n))
    setCallCounts((prev) => ({ ...prev, [n]: (prev[n] || 0) + 1 }))
    setLastCalled(n)
  }
  ...
  const uncallNumber = (n) => {
    ...
    if (calledThisRound.has(n)) {
      ...
      setLastCalled((prev) => (prev === n ? null : prev))
  ...
  const startNewRound = () => {
    setPreviousRounds((prev) => {
      const next = new Set(prev)
      calledThisRound.forEach((n) => next.add(n))
      return next
    })
  ...
  const handleRoundLimitChange = (e) => {
    ...
    setRoundLimit(Math.min(MAX_ROUND_LIMIT, Math.max(MIN_ROUND_LIMIT, val)))
  }
          ...
          {numbers.map((n) => {
            ...
            return (
              <button
                ...
                title={`Number ${n}${count > 0 ? ` — called ${count}x` : ''}`}
              >
